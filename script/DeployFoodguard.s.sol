...
contract DeployFoodguard is Script {
    ...
    function run() external returns (DeployedContracts memory, HelperConfig) {
        ...
        // 2. 部署所有模块合约
        DeployedContracts memory contracts = _deployAllContracts(config.account);
        ...
        // 5. 为各模块设置治理合约地址
        contracts.fundManager.setGovernanceContract(address(contracts.governance));
        contracts.poolManager.setGovernanceContract(address(contracts.governance));
        contracts.rewardManager.setGovernanceContract(address(contracts.governance));
        contracts.votingManager.setGovernanceContract(address(contracts.governance));
     ...
     */
    function _deployAllContracts(address admin) internal returns (DeployedContracts memory contracts) {
        // 部署模块合约
     ...
     */
    function _registerInitialUsers(DeployedContracts memory contracts) internal {
        // 这里可以注册一些测试用户
     ...
     */
    function verifyDeployment(DeployedContracts memory contracts) 
        external 
        view 
        returns (bool success, string[] memory issues) 
    {
