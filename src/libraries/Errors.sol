...
library Errors {
    ...
    /// @notice 验证者声誉不足
    error InsufficientValidatorReputation(address validator, uint256 current, uint256 required);
    ...
    /// @notice 事件时间不能晚于投诉时间
    error IncidentTimeAfterComplaint(uint256 incidentTime, uint256 complaintTime);
    ...
    /// @notice 配置超出范围
    error ConfigurationOutOfRange(string parameter, uint256 value, uint256 min, uint256 max);
