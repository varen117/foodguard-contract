...
contract RewardPunishmentManager {
    ...
    function processCaseRewardPunishment(
        ...
        if (validators.length != validatorChoices.length) {
            revert Errors.InvalidAmount(validators.length, validatorChoices.length);
        }
        
        if (challengers.length != challengeSuccessful.length) {
            revert Errors.InvalidAmount(challengers.length, challengeSuccessful.length);
        }
        ...
        // 处理验证者奖惩
        _processValidatorRewards(caseId, validators, validatorChoices, complaintUpheld);
        
        // 处理质疑者奖惩
        _processChallengerRewards(caseId, challengers, challengeSuccessful);
        
        // 处理投诉者和企业奖惩
        _processComplainantEnterpriseRewards(caseId, complainant, enterprise, complaintUpheld, riskLevel);
    ...
    function _processValidatorRewards(
        ...
        for (uint256 i = 0; i < validators.length; i++) {
            ...
            if (complaintUpheld && choice == DataStructures.VoteChoice.SUPPORT_COMPLAINT) {
                votedCorrectly = true;
            } else if (!complaintUpheld && choice == DataStructures.VoteChoice.REJECT_COMPLAINT) {
                votedCorrectly = true;
            }
            ...
            if (votedCorrectly) {
                ...
                status.rewardPunishment = DataStructures.RewardPunishmentStatus.REWARD;
                ...
                status.rewardPunishment = DataStructures.RewardPunishmentStatus.PUNISHMENT;
                
                // 计算惩罚金额
                uint256 punishmentAmount = _calculateValidatorPunishment(validator);
    ...
    function _processChallengerRewards(
        ...
        for (uint256 i = 0; i < challengers.length; i++) {
            ...
            if (successful) {
                ...
                uint256 punishmentAmount = _calculateChallengerPunishment(challenger);
    ...
    function _processComplainantEnterpriseRewards(
        ...
        if (complaintUpheld) {
            ...
            // 投诉者获得赔偿
            DataStructures.UserStatus storage complainantStatus = userStatuses[complainant];
            complainantStatus.integrity = DataStructures.IntegrityStatus.HONEST;
            complainantStatus.rewardPunishment = DataStructures.RewardPunishmentStatus.REWARD;
            ...
            // 企业接受惩罚
            DataStructures.UserStatus storage enterpriseStatus = userStatuses[enterprise];
            enterpriseStatus.integrity = DataStructures.IntegrityStatus.DISHONEST;
            enterpriseStatus.rewardPunishment = DataStructures.RewardPunishmentStatus.PUNISHMENT;
            ...
            // 投诉者接受惩罚（虚假投诉）
            DataStructures.UserStatus storage complainantStatus = userStatuses[complainant];
            complainantStatus.integrity = DataStructures.IntegrityStatus.DISHONEST;
            complainantStatus.rewardPunishment = DataStructures.RewardPunishmentStatus.PUNISHMENT;
            
            uint256 falseComplaintPenalty = _calculateFalseComplaintPenalty(riskLevel);
            ...
            enterpriseStatus.rewardPunishment = DataStructures.RewardPunishmentStatus.REWARD;
     ...
     * @notice 计算验证者奖励
     */
    function _calculateValidatorReward(address validator) internal view returns (uint256) {
        DataStructures.UserStatus storage status = userStatuses[validator];
     ...
     * @notice 计算验证者惩罚
     */
    function _calculateValidatorPunishment(address validator) internal view returns (uint256) {
        // 基础惩罚
     ...
     * @notice 计算质疑者奖励
     */
    function _calculateChallengerReward(address challenger) internal pure returns (uint256) {
     ...
     * @notice 计算质疑者惩罚
     */
    function _calculateChallengerPunishment(address challenger) internal pure returns (uint256) {
     ...
     * @notice 计算赔偿金额
     */
    function _calculateCompensation(DataStructures.RiskLevel riskLevel) internal pure returns (uint256) {
     ...
     * @notice 计算企业惩罚
     */
    function _calculateEnterprisePenalty(DataStructures.RiskLevel riskLevel) internal pure returns (uint256) {
        if (riskLevel == DataStructures.RiskLevel.HIGH) {
            return 5 ether;
        } else if (riskLevel == DataStructures.RiskLevel.MEDIUM) {
            return 2 ether;
     ...
     * @notice 计算虚假投诉惩罚
     */
    function _calculateFalseComplaintPenalty(DataStructures.RiskLevel riskLevel) internal pure returns (uint256) {
        if (riskLevel == DataStructures.RiskLevel.HIGH) {
            return 0.5 ether;
        } else if (riskLevel == DataStructures.RiskLevel.MEDIUM) {
            return 0.2 ether;
     ...
     * @notice 计算声誉恢复补偿
     */
    function _calculateReputationCompensation() internal pure returns (uint256) {
        return 0.05 ether;
    }
     ...
     * @notice 获取案件奖惩记录
     */
    function getCaseRewardRecord(uint256 caseId) external view returns (
        uint256,    // caseId
        bool,       // complaintUpheld
        uint8,      // riskLevel
     ...
     * @notice 获取个人奖励信息
     */
    function getPersonalReward(uint256 caseId, address user) external view returns (PersonalReward memory) {
     ...
     * @notice 获取个人惩罚信息
     */
    function getPersonalPunishment(uint256 caseId, address user) external view returns (PersonalPunishment memory) {
     ...
     * @notice 获取用户状态
     */
    function getUserStatus(address user) external view returns (DataStructures.UserStatus memory) {
     ...
     * @notice 更新用户声誉分数
     */
    function updateUserReputation(address user, uint256 newScore) external onlyGovernance {
