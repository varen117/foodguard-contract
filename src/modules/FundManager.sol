...
contract FundManager is AccessControl, ReentrancyGuard, Pausable {
    ...
    function batchDistributeRewards(
        ...
        if (fundPool.rewardPool < totalAmount) {
            revert Errors.InsufficientFundPool(
                totalAmount,
                fundPool.rewardPool
            );
        }
    ...
    function emergencyWithdraw(
        ...
        address recipient,
        string calldata reason
    ) external onlyRole(DEFAULT_ADMIN_ROLE) notZeroAddress(recipient) {
    ...
    function pause() external onlyRole(DEFAULT_ADMIN_ROLE) {
        ...
        emit Events.SystemPauseStatusChanged(true, msg.sender, block.timestamp);
    }
