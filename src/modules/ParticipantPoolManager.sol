...
contract ParticipantPoolManager is Ownable, CommonModifiers {
    ...
    mapping(DataStructures.UserRole => mapping(address => uint256))
        public userPoolIndex;

    /// @notice 用户在角色池中是否存在 role => user => exists
    mapping(DataStructures.UserRole => mapping(address => bool))
        public isInRolePool;
    ...
    mapping(uint256 => mapping(address => DataStructures.UserRole))
        public caseUserRole;
    ...
    event UserRoleUpdated(
        address indexed user,
        DataStructures.UserRole oldRole,
        DataStructures.UserRole newRole
    );
    event ValidatorsSelected(uint256 indexed caseId, address[] validators);
    event ChallengersSelected(
    ...
    ) external onlyGovernance returns (address[] memory validators) {
        // 验证参数
        require(randomWords.length > 0, "Random words cannot be empty");

        // 获取可用的DAO成员作为验证者（排除已参与此案件的用户）
        address[] memory availableValidators = _getAvailableParticipants(
            caseId,
    ...
    function _getAvailableParticipants(
        ...
        DataStructures.UserRole role,
        uint256[] calldata randomWords
    ) internal returns (address[] memory available) {
        address[] storage rolePool = rolePools[role];
        uint256 poolSize = rolePool.length;
        uint256 randomWordsLength = randomWords.length;

        // 预分配结果数组
        available = new address[](randomWordsLength);

        // 使用单个遍历，直接构建可用用户数组
        address[] memory availableUsers = new address[](poolSize);
        ...
        for (uint256 i = 0; i < poolSize; i++) {
            ...
            if (isUserActive[user] && !caseParticipation[caseId][user]) {
                availableUsers[availableCount] = user;
                availableCount++;
            }
        }
        ...
        for (uint256 i = 0; i < randomWordsLength; i++) {
            uint256 selectedIndex = randomWords[i] % availableCount;
            address selectedUser = availableUsers[selectedIndex];
            available[i] = selectedUser;

            // 记录参与状态
            caseParticipation[caseId][selectedUser] = true;
            caseUserRole[caseId][selectedUser] = role;
        }
     ...
     */
    function _getAvailableChallengersForValidator(
        uint256 caseId,
        address targetValidator
    ) internal view returns (address[] memory available) {
        address[] storage daoMembers = rolePools[
            DataStructures.UserRole.DAO_MEMBER
        ];
        uint256 availableCount = 0;
     ...
     */
    function getUserCaseParticipation(
        uint256 caseId,
        address user
    )
        external
    ...
    {
        return (caseParticipation[caseId][user], caseUserRole[caseId][user]);
    }

    /**
     * @notice 检查企业是否已注册
     */
    function isEnterpriseRegistered(
        address enterprise
    ) external view returns (bool) {
        return
            isRegistered[enterprise] &&
            userRoles[enterprise] == DataStructures.UserRole.ENTERPRISE;
    }
    ...
    function getTotalUsers() external view returns (uint256) {
        return
            rolePools[DataStructures.UserRole.COMPLAINANT].length +
    ...
    function updateValidatorConfig(
        ValidatorConfig calldata newConfig
    ) external onlyOwner {
        require(newConfig.minValidators % 2 == 1, "Min validators must be odd");
        require(
            newConfig.defaultValidators % 2 == 1,
            "Default validators must be odd"
        );
        require(
            newConfig.minValidators <= newConfig.defaultValidators,
            "Invalid config"
        );
        require(
            newConfig.defaultValidators <= newConfig.maxValidators,
            "Invalid config"
        );
     ...
     * @notice 更新质疑者配置
     */
    function updateChallengerConfig(
        ChallengerConfig calldata newConfig
    ) external onlyOwner {
        require(
            newConfig.minChallengerReputation > 0,
            "Invalid min reputation"
        );
     ...
     */
    function resetCaseParticipation(
        uint256 caseId,
        address[] calldata users
    ) external onlyOwner {
        for (uint256 i = 0; i < users.length; i++) {
