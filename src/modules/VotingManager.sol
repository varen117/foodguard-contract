...
contract VotingManager {
    ...
    function updateValidatorStatus(
        ...
        DataStructures.ValidatorInfo storage validator = validators[validatorAddress];
    ...
    function startVotingSession(
        ...
        if (selectedValidators.length < requiredValidators) {
            revert Errors.InsufficientValidators(selectedValidators.length, requiredValidators);
        }
    ...
    function submitVote(
        ...
        _validateVoteSubmission(caseId, reason, evidenceHashes, evidenceTypes, evidenceDescriptions);
        _recordVoteAndEvidence(caseId, choice, reason, evidenceHashes, evidenceTypes, evidenceDescriptions);
    ...
    function _recordVoteAndEvidence(
        ...
        for (uint256 i = 0; i < evidenceHashes.length; i++) {
            session.votes[msg.sender].evidences.push(DataStructures.Evidence({
     ...
     * @notice 更新投票统计
     */
    function _updateVotingStatistics(uint256 caseId, DataStructures.VoteChoice choice) internal {
        VotingSession storage session = votingSessions[caseId];
        ...
        returns (bool complaintUpheld) 
    {
        ...
        for (uint256 i = 0; i < session.selectedValidators.length; i++) {
            ...
            if (vote.hasVoted) {
                bool votedWithMajority = (complaintUpheld && vote.choice == DataStructures.VoteChoice.SUPPORT_COMPLAINT) ||
                                       (!complaintUpheld && vote.choice == DataStructures.VoteChoice.REJECT_COMPLAINT);
     ...
     * @return 选中的验证者地址数组
     */
    function _selectRandomValidators(uint256 count) internal returns (address[] memory) {
        // 获取活跃验证者
     ...
     * @return 是否为选中的验证者
     */
    function _isSelectedValidator(uint256 caseId, address validator) internal view returns (bool) {
        VotingSession storage session = votingSessions[caseId];
     ...
     * @return 投票信息
     */
    function getValidatorVote(uint256 caseId, address validator) 
        external 
        view 
    ...
    function getValidatorInfo(address validator) 
        external 
        view 
        returns (DataStructures.ValidatorInfo memory) 
    {
